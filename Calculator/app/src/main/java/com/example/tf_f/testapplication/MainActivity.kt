...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        equal.setOnClickListener {
            Value_Output.text = "${Value_Output.text}="
            addList(nStr,'=')
            var v_result = calcualte()
            val result_ : Int

            if(v_result % 1.0 == 0.0) {
                result_ = v_result.toInt()
                Value_Output.text = result_.toString()
                nStr = result_.toString()
            }else {
                Value_Output.text = v_result.toString()
                nStr = v_result.toString()
            }
